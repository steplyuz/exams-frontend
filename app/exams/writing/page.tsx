...
import { PenLine, Clock, ChevronRight, ChevronLeft, Search, Inbox, ListChecks } from "lucide-react"

import { Badge } from "@/components/ui/badge"
import { getAllWritingExamsAPI } from "@/lib/cefr-api/writing"

interface WritingExamItem {
  id: string
  title: string
  isActive: boolean
  level: string
...
export default function WritingListPage() {
  ...
  const [exams, setExams] = useState<WritingExamItem[]>([])
  ...
  useEffect(() => {
    const fetchExams = async () => {
      try {
        ...
        const formatted: WritingExamItem[] = rawData
  ...
  const handleTestClick = (test: WritingExamItem) => {
    ...
    router.push(`/exams/writing/start?id=${test.id}`)
  }
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 px-4 pb-10 pt-6 sm:px-6">
      ...
      <div className="rounded-3xl border border-border bg-card p-7 shadow-sm sm:p-9">
        ...
        <h1 className="mt-5 text-2xl font-bold tracking-tight sm:text-3xl">Writing</h1>
        <p className="mt-2 max-w-lg text-sm leading-6 text-muted-foreground">
          Sizga tayinlangan yozish topshiriqlarini shu yerdan boshlang.
        </p>
      </div>
      ...
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="Mavzu yoki darajasini qidiring..."
          value={searchTerm}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="mt-1.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs font-medium text-muted-foreground">
                      ...
                      <span className="flex items-center gap-1.5">
                        <ListChecks size={13} /> {test.taskCount} topshiriq
                      </span>
                    </div>
                  </div>
                </div>
