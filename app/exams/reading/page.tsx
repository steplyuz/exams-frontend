...
import { BookOpen, Clock, FileText, ChevronRight, ChevronLeft, Search, Inbox } from "lucide-react"

import { Badge } from "@/components/ui/badge"
import { getAllReadingExamsAPI } from "@/lib/cefr-api/reading"
...
export default function ReadingPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const formatted: ExamCardItem[] = rawData
        ...
        console.error("Reading testlarni yuklashda xatolik:", error)
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleTestClick = (test: ExamCardItem) => {
    ...
    router.push(`/exams/reading/start?id=${test.id}`)
  }
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 px-4 pb-10 pt-6 sm:px-6">
      ...
      <div className="rounded-3xl border border-border bg-card p-7 shadow-sm sm:p-9">
        ...
        <h1 className="mt-5 text-2xl font-bold tracking-tight sm:text-3xl">Reading</h1>
        <p className="mt-2 max-w-lg text-sm leading-6 text-muted-foreground">
          Sizga tayinlangan o'qish testlarini shu yerdan boshlang.
        </p>
      </div>
